...
When we consider the sentence so far is `Motorola Xoom commercial reminiscent of Apple's`, the top 2 tokens (new and first) are suitable choices.  
...
The model must now figure out how to start the new sentence. 

:::{figure} #token_predictions_two
:label: label_token_predictions_two
:::

[](#label_token_predictions_two) shows the probability distribution for this specific case. As a start of the sentence, any of the top 10 choises would make a solid start to a new sentence. 

This specific example highlights the biggest problem I have been facing so far, where I can keep trying to optimise this model while we are actually just missing important context. The next word we are looking for in the sentence is 'Snapchat', however there is no way for the model to possibly figure this out, let alone what happens to snapchat next.

# Concluding thoughts 

With a final validation loss of $1.25$, further improvements are most likely to come from adding more context. One way of doing this could be by providing the article itself for the model to extract keywords like company names, sentiment, etc. 
